...
**Durée** : ~20 minutes pour un nouveau projet, ~10 minutes pour une mise à jour.
...
│   4. Installation Husky + hook gitleaks                 │
│   5. Workflow GitHub Actions (CI sécurité)              │
│   6. Branch protection (main, staging)                  │
│   7. GitHub Secret Scanning                             │
│   8. Setup Supabase (RLS dès le début)                  │
│   9. Setup Vercel et/ou Render                          │
│   10. Test final : tentative de commit piégée           │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
## 4. Husky + hook gitleaks pre-commit
...
## 5. Workflow GitHub Actions — CI de sécurité
...
Si tu veux le tester en cas de fuite, tente de pousser le commit piégé de l'étape 4 (avec `--no-verify` pour bypass le hook local) et regarde le workflow Actions s'exécuter et échouer.
...
## 7. GitHub Secret Scanning (optionnel — selon le plan de l'org)
...
- Les secrets au moment du commit local (étape 4)
- Les secrets dans les PR avant merge (étape 5)
...
**À faire uniquement si l'app utilise Supabase.** Si pas de Supabase, passe à l'étape 9.
...
## 10. Test final — Vérifier que tout fonctionne
...
**Supabase refuse mes requêtes après avoir activé RLS** : c'est normal, le DENY ALL par défaut bloque tout. Crée des policies explicites pour chaque opération autorisée (cf. exemples étape 8).
